 ...
 *
 * Covers the decisions in docs/meetings-scope.md and docs/meeting-pages-scope.md:
 * server-rendered URL filters, day grouping, meaningful action names, cards
 * linking to meeting pages, location pages redirecting, and no plugin
 * front-end assets. The meeting page itself: npm run a11y:meeting.
 */
...
const read = async (page) => page.evaluate(() => {
  ...
  const cta = [...document.querySelectorAll('.ma-meeting__actions a')];
  const names = [...document.querySelectorAll('.ma-meeting__name a')];
  const details = [...document.querySelectorAll('.ma-meeting__cta')];
  return {
    ...
    nameHrefs: names.map(a => a.getAttribute('href')),
    detailHrefs: details.map(a => a.getAttribute('href')),
    zoomLogos: [...document.querySelectorAll('.ma-meeting__join svg')].map(svg => svg.getAttribute('aria-hidden')),
...
check('every action names its meeting', all.ctaNames.every(n => /^Join .+ on .+/.test(n) || /^Meeting details for .+/.test(n)),
  all.ctaNames.find(n => n.startsWith('Join')) + ' | ' + all.ctaNames[0]);
check('each meeting name links to its page', all.nameHrefs.length === all.meetings && all.nameHrefs.every(h => /\/meetings\/[^/?#]+\/$/.test(h)),
  `${all.nameHrefs.length} names, e.g. ${all.nameHrefs[0]}`);
check('"Meeting details" goes where the name goes', all.detailHrefs.length === all.meetings && all.detailHrefs.every((h, i) => h === all.nameHrefs[i]),
  `${all.detailHrefs.length} links`);
check('Zoom logo is decorative', all.zoomLogos.length > 0 && all.zoomLogos.every(v => v === 'true'), `${all.zoomLogos.length} logos`);
const onlyAllowed = all.allHrefsInMain.every(h =>
  h.includes('?type=') || /\/meetings\//.test(h) || h.includes('zoom.us') || h.includes('/contact/'));
check('no unexpected links (no Google Maps)', onlyAllowed, `${all.allHrefsInMain.length} links checked`);
...
// --- Meeting pages, location redirects and sitemap ------------------------------
const single = await fetch(new URL(all.nameHrefs[0], LIST), { redirect: 'manual' });
const loc = await fetch(`${BASE}/locations/`, { redirect: 'manual' });
check('meeting page URL serves a page (200)', single.status === 200, `${single.status} ${all.nameHrefs[0]}`);
...
check('location pages absent from Yoast sitemap', sitemap === '' || !/tsml_location|location-sitemap/.test(sitemap),
